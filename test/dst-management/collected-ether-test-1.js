...
/**
 *
 * Testing that collectedEther is not subject to manipulation.
 */
...
it('send-ether-to-dst-no-overflow', function() {
    log("");
    log(" (!) Action: [0x2980] send 50 ether to [APL]");
    var collectedEther = dstContract_APL.getCollectedEther();
    var accBalance = sandbox.web3.eth.getBalance('0x29805ff5b946e7a7c5871c1fb071f740f767cf41');
    log("[APL] => collectedEther: " + collectedEther);
    log("[0x2980] => balance: " + accBalance);

    return workbench.sendTransaction({
      from: '0x29805ff5b946e7a7c5871c1fb071f740f767cf41',
      to: dstContract_APL.address,
      gas: 200000,
      value: sandbox.web3.toWei(50, 'ether')
    })
    ...
    .then(function () {
        ...
        collectedEther = dstContract_APL.getCollectedEther() - collectedEther;
        log("[APL] => ether collected: " + collectedEther);
        var ethSpent = accBalance - sandbox.web3.eth.getBalance('0x29805ff5b946e7a7c5871c1fb071f740f767cf41');
        log("[0x2980] => actual ether spent: " + ethSpent);
        assert(Math.abs(ethSpent-collectedEther) < 5000000000000000);
...
it('send-ether-to-dst-overflow', function() {
    log("");
    log(" (!) Action: [0x2980] send 5000 ether to [APL]");
    var collectedEther = dstContract_APL.getCollectedEther();
    var accBalance = sandbox.web3.eth.getBalance('0x29805ff5b946e7a7c5871c1fb071f740f767cf41');
    log("[APL] => collectedEther: " + collectedEther);
    log("[0x2980] => balance: " + accBalance);
